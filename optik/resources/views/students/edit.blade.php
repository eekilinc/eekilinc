<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Öğrenciyi Düzenle</h2>
    </x-slot>

    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('classes.students.update', [$class, $student]) }}" method="POST" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <x-input-label for="student_no" value="Öğrenci No" />
                        <x-text-input id="student_no" name="student_no" class="mt-1 block w-full" value="{{ old('student_no', $student->student_no) }}" required />
